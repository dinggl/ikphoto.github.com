<%@ page language="java" import="java.util.*,com.baidu.scan.model.*"
	pageEncoding="UTF-8"%>
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ path + "/";
	List<Folder> folders = (List<Folder>) request
...
.radiomark {
    ...
    var util = {currentPage:<%=currentPage%>,
		...
		setCurrentPage : function(currentPage) {
			...
			scroll.animate({
				scrollLeft : (this.currentPage - 1) * 880
			}, 1000);
	...
	$()
			.ready(
                   ...
                   +'"class="folder_img" name="unbind" src="../images/folder-gray.png"></div>'
									+ '<div class="folder_desc"><span class="name" name="unbind" >'
									+ msg.name
									+ '</span><input class="folder_input" name="unbind" type="text"><span class="size" >'
									+ msg.size + '张</span></div>' + '</li>';
						}
						function getFolderColumn(msg) {
							return '<li class="lione">'
									+ '<ul class="folder_column">'
									+ getFolderCell(msg) + '</ul>' + '</li>';
						}
						function folder_img_mouseover() {
							var temp = $(this);
							temp.attr("src", "../images/folder-open.png");
							temp.parent().parent().css("background", "#F0F1F4")
									.css("border", "1px solid black");
						}
						function folder_img_mouseout() {
							var temp = $(this);
							temp.attr("src", "../images/folder-gray.png");
							temp.parent().parent().css("background", "white")
									.css("border", "0px");
						}
						function folder_img_click() {
							window.location.href = "folder_photo?folderid="
									+ $(this).attr("folderid");
						}
						function folder_name_click() {
							...
							var value = temp.html();
							temp.parent().children().eq(1).val(value).css(
									"display", "block").focus();
						}
						function folder_input_blur() {
							...
							$.ajax(
									...
									function(msg) {
										temp.css("display", "none");
										temp.parent().children().eq(0).html(
						...
						function marktext_blur() {
							...
							temp.css("display", "none");
							var value = temp.val();
							temp.parent().children().eq(0).html(value).css(
									"display", "block");
						}
						function radiocheck() {
							var _this = $(this);
						...
						}).mouseover(folder_img_mouseover).mouseout(
								folder_img_mouseout).click(folder_img_click)
								.attr("name", "bind");
						...
						$("#left").click(function() {
							util.left();
								...
								function() {
									$(this).css("background-color", "#F0F1F4")
											.css("border", "1px solid black");
								}).mouseout(
								function() {
							...
							util.drag = true;
								...
								.mousemove(
										function(e) {
											...
											if (util.drag) {
												var _this = $(this);
												var left = parseInt(_this
														.css("left"))
														+ e.clientX - util.x;
												if (left + 700 < $(document)
														.width() - 10
														&& left > 10) {
													_this.css("left", left
															+ "px");
						...
						$("#communicate")
								.click(
										function() {
											var frame = document
													.getElementById('frame').contentWindow.document;
											var uuid = $(
													frame
															.getElementById("file_form"))
													.attr("uuid");
						...
						$('.member')
								.hover(
										function() {
											...
											_this
													.css('background',
															'url(../images/down.png) no-repeat 130px center')
													.css("overflow", "visible");

											$(_this.children().eq(0)).show()
													.animate({
														opacity : 100,
														height : 50
													});
										...
										function() {
											...
											_this
													.css('background',
															'url(../images/up.png) no-repeat 130px center');
	...
	<body>
		...
		<div id="uploader">
			...
			<div id="button">
				<iframe id="frame" name="frame" src="uploader.jsp"></iframe>
				<input type="button" id="communicate" style="display: none;">
				<div id="saveas">
					<select id="as"></select>
		...
		<div id="folder">
						...
						<ul class="folder_column">
							...
							<li>
								<div class="img_container">
									<img class="folder_img" folderid="<%=folder.getId()%>"
										name="unbind" src="../images/folder-gray.png">
								</div>
								<div class="folder_desc">
									<span class="name" name="unbind"><%=folder.getName()%></span>
									<input class="folder_input" name="unbind" type="text">
									<span class="size"><%=folder.getSize()%>张</span>
								</div>
							</li>
							<%
								count++;
			...
			<div id="nav">
				<img id="left" src="../images/arrow-left.png">
				<img id="right" src="../images/arrow-right.png">
